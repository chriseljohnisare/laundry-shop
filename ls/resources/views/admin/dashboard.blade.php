<x-admin-layout>
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 space-y-4 md:space-y-0">
        ...
        <div class="flex items-center space-x-3">
            ...
            <a href="{{ route('admin.orders.create') }}" class="bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
        ...
        <div class="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-xl transition-all duration-300">
            ...
            <h3 class="text-3xl font-black text-gray-900">${{ number_format($revenue, 2) }}</h3>
        </div>
        ...
        <div class="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-xl transition-all duration-300">
            ...
            <h3 class="text-3xl font-black text-gray-900">{{ number_format($pendingOrders) }}</h3>
        </div>
        ...
        <div class="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group hover:shadow-xl transition-all duration-300">
            ...
            <h3 class="text-3xl font-black text-gray-900">{{ number_format($completedOrders) }}</h3>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8 border-b border-gray-50 flex justify-between items-center">
                ...
                <a href="{{ route('admin.orders.index') }}" class="text-indigo-600 font-bold text-sm hover:text-indigo-800 transition">View Ledger →</a>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($recentOrders as $order)
                            <tr class="hover:bg-gray-50/80 transition-colors">
                                <td class="px-8 py-5">
                                    <span class="bg-gray-100 text-gray-700 px-2 py-1 rounded-lg text-xs font-mono font-bold">#{{ $order->receipt_number }}</span>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm font-bold text-gray-700">{{ $order->customer->name }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-sm text-gray-500 font-medium">{{ $order->service->name }}</td>
                                <td class="px-8 py-5">
                                    @php
                                        $statusConfig = [
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider {{ $conf['color'] }}">
                ...
                @foreach ($ordersByService as $service)
                    <div>
                        <div class="flex justify-between items-center mb-3">
                            <span class="text-xs font-black text-gray-500 uppercase tracking-widest">{{ $service->name }}</span>
                            <span class="text-sm font-black text-gray-900">{{ number_format($service->count) }} ops</span>
                        </div>
                        @php
